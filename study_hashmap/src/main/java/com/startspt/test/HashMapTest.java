...
/**
 * 一、Map实现类的结构：
 *  /---- Map:双列数据，存储key-value对的数据，----类似高中时的函数：y=f(x)
 ...
 *      的哈希值：
 *              如果key1的哈希值与已经存在的数据的哈希值都不相同，此时的key1-value1(Entry)添加成功。----情况2
 *              如果key1的哈希值与已经存在的某一个数据（key2-value2）的哈希值相同，继续比较：调用key1所在类的equals(key2)
 *                      如果equals()返回false:此时的key1-value1(Entry)添加成功。----情况3
 *                      如果equals()返回true:使用value1代替value2.
 *
...
public class HashMapTest {
    ...
    public void test5() {
        ...
        System.out.println("遍历所有的key集：keySet()");
        Set set = map.keySet();
        ...
        System.out.println("遍历所有的value集：values()");
        Collection values = map.values();
        for(Object obj :values){
            System.out.println(obj);
        }

        System.out.println("遍历所有的key-value:entrySet()");
